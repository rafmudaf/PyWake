...
def pywasp_pwc_point():
    ...
    ds['not_needed'] = ds['A'] * np.random.random(ds['A'].shape)
    return ds
...
@pytest.mark.parametrize('method_speedup,ref', [('park', 0),
                                                ('global_weibull', 1),
                                                ])
def test_from_pwc_point(method_speedup, ref, pywasp_pwc_point):

    site = XRSite.from_pwc(pwc=pywasp_pwc_point, method_speedup=method_speedup, drop_vars=['not_needed'])
    ...
    A = (
        ...
        np.array([5.757, 5.584, 7.506, 10.082, 9.644, 7.066, 6.265, 7.562,
                  10.124, 11.211, 9.624, 6.499])
    )[ref]
    ...
    k = (
        ...
        np.array([1.919922, 2.166016, 2.638672, 3.044922, 2.880859, 2.666016,
                  2.501953, 2.529297, 2.533203, 2.333984, 1.986328, 1.767578])
    )[ref]
    ...
    speedups = (
        ...
        np.array([[1.00012085, 1.00000037, 1., 0.93050706, 0.91616416,
                   0.91480328, 1.00011677, 1., 0.98824575, 0.89596567,
                   0.91656276, 0.91521773, 1.00012085],
                  [1.05749522, 1.03975645, 1.02663576, 1., 1.,
                   1.00070761, 1.05762171, 1.03935626, 1.02927942, 1.,
                   1., 1.00076935, 1.05749522],
                  [0.93179214, 0.95952751, 0.9472372, 1.09283872, 1.05959859,
...
def test_from_pwc_cuboid(pywasp_pwc_cuboid):

    site = XRSite.from_pwc(pywasp_pwc_cuboid)
